...
Let's learn about `EXPOSE` and how it's different from `publish`:

The EXPOSE directive in Dockerfile primarily serves as documentation, signaling to users which port the containerized application is set to run on. Interestingly, while EXPOSE will make the port available to other containers, it does not make that port accessible to the host machine or the external world.

The --publish or -p flag used with `docker run` is what truly opens up a port to external traffic. If you run a container without publishing a port, even if the Dockerfile contains an EXPOSE directive, the port won't be accessible from outside the Docker container.

As an example, if we take a look at the [Nginx official Dockerfile](https://github.com/nginxinc/docker-nginx/blob/dcaaf66e4464037b1a887541f39acf8182233ab8/mainline/debian/Dockerfile), we'll see that port 80 is exposed with `EXPOSE 80`. This only means that the Nginx application will run on port 80 of the container.

However, if we want to publish the Nginx application to a port on the host machine, we can do that by using the `--publish` option. 

References:

[Blog](https://dev.to/sudhz_/expose-vs-publish-in-docker-a-deeper-dive-457g#:~:text=Internal%20vs%20External%3A%20EXPOSE%20makes,bind%20and%20open%20those%20ports.)

[Baeldung](https://www.baeldung.com/ops/docker-expose-vs-publish)

Question: Can we run multiple applications on the same Docker container? (as in, can we run different applications on different ports, but on the same container?)

Answer: Yes. although not very recommended.
...
Reference: [Docker Documentation](https://docs.docker.com/engine/containers/multi-service_container/)

## Before we go ahead, let's discuss about few problems. This will set the context for bind mounts and volumes

1. Docker containers are ephemeral in nature, which means that all the data generated by the containers are lost once they are terminated. Let's consider a usecase where an Nginx application is running on a container. We might want to persist the logs generated by Nginx application, even after the container is terminated.

2. Let's say we have two running containers - on one of these containers a backend service is running and on the other one, a frontend web app is running. Let's say that the service generates some data that the web app needs to consume. In this case, we need a mechanism to share data between different containers.

3. Let us say there is a cron job running on the host which periodically writes some data, and we want a container to consume this data. Again, we need a mechanism for data sharing between host and docker containers.

In order to solve the problems listed above, Docker provides for two solutions:

1. Bind mounts: This essentially means that we mount a directory from host filesystem to the container. So, the container would actually be leveraging the mounted directory from host filesystem.

2. Volumes: Creating docker volumes essentially means creating a logical volume/drive on the host filesystem. This volume can be attached to one or more containers. It's important to note that volumes need not always be on the local filesystem, even mounting external volumes like S3 buckets are possible.

Here's a table which outlines the differences between volume and bind mounts:

![Differences-between-volume-and-bind-mount](./Differences-between-volume-and-bind-mount.png)

There are two differents ways to create a bind/volume mount in Docker, either using the `--v` or `--mount` options. It is preferable to use the `--mount` option as it is verbose.

[Reference Video](https://www.youtube.com/watch?v=r_LgmqejAkA)

Let's understand about bind mounts and volume in detail:

## Let's explore bind mount ##

Supppose that I make any change in the local project on my host machine, those changes won't reflect in the docker container's project. But if I have a usecase where we want that any changes made on the host machine should reflect in the Docker container as well, then we can use the bind mount. Bind mount enables a two way mapping between the Docker container and the host machine, meaning that any changes made in the host machine will reflect in the Docker container and vice versa.

In order to use the bind mount, we have to specify a mapping between the directory in the host machine and the directory in the Docker container using the `-v` option. As an example, consider the syntax: `docker run -it --init -p 3002:3000 -v "$(pwd)":/developer/nodejs/node-bind-mount-project app-bind-mount-node:latest`
...
## Let's explore about Docker volume ##

Docker volumes are used to persist the data of the Docker container, even after the container is shut down or exited. They can also be used to share data between containers.

In order to demonstrate the concept of Docker volumes, we'll create a nodejs application with the capability to read and print data from logs folder. 

Initially, the logs folder will be empty, and we will populate some dummy text in the logs folder. Post that, we'll kill the container and boot up container of the same image again.

Since we are using `volume mount`, the contents of logs folder will be read from the docker volume. This proves the usecase of Docker volume to persist data.

![Docker-volume-mount-demo](./Docker-volume-mount-demo.png)
